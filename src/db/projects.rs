...
use crate::db::util::{DBResult, DbDate};
...
impl DBProject {
    pub(crate) async fn get_ordered_by_id(conn: &mut PgConnection) -> DBResult<Vec<DBProject>> {
        sqlx::query_as!(DBProject, r#"SELECT * FROM "project" ORDER BY id ASC;"#).fetch_all(conn).await
    }
    pub(crate) async fn get(conn: &mut PgConnection) -> DBResult<Vec<DBProject>> {
        sqlx::query_as!(DBProject, r#"SELECT * FROM "project" ORDER BY "default" DESC, active DESC, id DESC;"#)
            .fetch_all(conn)
            .await
    }

    pub(crate) async fn get_by_id(project_id: i64, conn: &mut PgConnection) -> DBResult<DBProject> {
        sqlx::query_as!(DBProject, r#"SELECT * FROM "project" WHERE id = $1 ORDER BY id ASC;"#, project_id,)
            .fetch_one(conn)
            .await
    }
    ...
    pub(crate) async fn delete(project_id: i64, conn: &mut PgConnection) -> DBResult<()> {
        sqlx::query!(r#"DELETE FROM "project" WHERE id=$1"#, project_id).execute(conn).await?;
        Ok(())
    }

    pub(crate) async fn set_default(project_id: i64, conn: &mut PgConnection) -> DBResult<()> {
        Self::clear_default(conn).await?;
        sqlx::query!(r#"UPDATE "project" SET "default" = true WHERE id = $1;"#, project_id,).execute(conn).await?;
        Ok(())
    }

    pub(crate) async fn clear_default(conn: &mut PgConnection) -> DBResult<()> {
        sqlx::query!(r#"UPDATE "project" SET "default" = false WHERE "default";"#).execute(conn).await?;
        Ok(())
    }
}
